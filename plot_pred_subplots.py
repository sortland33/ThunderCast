...
def get_abifile_lists(rootdatadir, dts): #get list of abifiles for each datatime for channels specified
    list = []
...
def get_abifile_dict(rootdatadir, dts): #get a dictionary of abi files for each datetime
    dict = {}
    abi_channels = ['02', '05', '11', '13', '14', '15'] #for this application we do not need all the channels
    for i in range(len(dts)):
        satdir = dts[i].strftime(rootdatadir + '/%Y/%Y_%m_%d_%j/abi/L1b/RadC/')
...
def format_inputs(scn, norm_vals, vars, center_x_idx, center_y_idx, hs1km, full_US): #format satellite inputs for using in model
    X = []
...
def load_pt(file, params): #load pt file
    X = torch.load(file)
...
class DataAugmentation(nn.Module): #set-up data augmentation
    """
...
class UNet2D_LitModel(pl.LightningModule): #define model
...
def get_radar_list(dts, rootradardir): #get a list of radar files for all datetimes
    radar_file_list = []
    for dt in dts:
...
def get_radar_dict(dts, rootradardir): #get radar dictionary for datetimes
    radar_file_dict = {}
    for dt in dts:
        radardir = dt.strftime(os.path.join(rootradardir, '%Y/%Y%m%d/Reflectivity_-10C/00.50/'))
        ###Grab radar files within 5 minutes of the ABI file and choose the first one to resample and plot.
        rad_start = dt - timedelta(minutes=2)
        rad_end = dt + timedelta(minutes=2)
        radfiles = {datetime.strptime(x.split('/')[-1], '%Y%m%d-%H%M%S.netcdf'): x for x in
                    glob(os.path.join(radardir, '*.netcdf'))}
        five_min_radfilelist = [radfiles[x] for x in radfiles if dt <= x <= rad_end]
...
def make_subplot(scn, preds, center_x_idx, center_y_idx, hs, radar_file, c, timestamp, i, j, fig, axes, geoproj, straight): #make figure
    ...
    if straight:
        ...
        axes[j].set_extent([xmin, xmax, ymin, ymax], crs=geoproj)
        axes[j].add_feature(cfeature.BORDERS)
        #axes[i, j].add_feature(state_borders, edgecolor='plum')

        if c == 'C05':
            img = axes[j].pcolormesh(xx1km, yy1km, C05vals, vmin=0, vmax=100, cmap=matplotlib.colormaps['Greys_r'])
            radimg_10C = axes[j].pcolormesh(xx1km, yy1km, ma.masked_less(radar_data, 10), cmap=CVD_cmap, norm=Normalize(-1, 80), linewidth=0, antialiased=True)#, alpha=0.9)
            ...
            contour_img = axes[j].contour(xx1km, yy1km, preds, values, colors=colors,
                                     linewidths=linewidths)  # , cmap=plt.get_cmap('cividis'))
            custom_lines = [Line2D([0], [0], color=c, lw=lw * 2) for c, lw in zip(colors, linewidths)]
        elif c == 'C13':
            img = axes[j].pcolormesh(xx1km, yy1km, zoom(C13vals, 2, order=0), vmin=200, vmax=300, cmap=matplotlib.colormaps['Greys'])
            radimg_10C = axes[j].pcolormesh(xx1km, yy1km, ma.masked_less(radar_data, 10), cmap=CVD_cmap, norm=Normalize(-1, 80), linewidth=0, antialiased=True)#, alpha=0.9)
            colors = ['#FF9494', '#FF5F5F', '#DC1C1C', '#920000']
            values = [0.1, 0.3, 0.5, 0.8]
            linewidths = [2.2, 2.4, 2.6, 3]
            ...
            img = axes[j].pcolormesh(xxhalfkm, yyhalfkm, C02vals, vmin=0, vmax=100, cmap=matplotlib.colormaps['Greys_r'])
            radimg_10C = axes[j].pcolormesh(xxhalfkm, yyhalfkm, ma.masked_less(zoom(radar_data, 2, order=0), 10), cmap=CVD_cmap, norm=Normalize(-1, 80), linewidth=0, antialiased=True)#, alpha=0.9)
            colors = ['#FF9494', '#FF5F5F', '#DC1C1C', '#920000']
            values = [0.2, 0.4, 0.6, 0.8]
            linewidths = [1.6, 1.6, 1.7, 1.8, 2]
            labels = [str(int(v * 100)) + '%' for v in values]
            contour_img = axes[j].contour(xxhalfkm, yyhalfkm, zoom(preds, 2, order=0), values, colors=colors,
                                     linewidths=linewidths)  # , cmap=plt.get_cmap('cividis'))
            custom_lines = [Line2D([0], [0], color=c, lw=lw * 2) for c, lw in zip(colors, linewidths)]
        axes[j].set_title((' ').join([timestamp, 'UTC']), fontsize=16)
    else:
        xmin = np.min(x1km);
        xmax = np.max(x1km);
        ymin = np.min(y1km);
        ...
        if c == 'C05':
            ...
            radimg_10C = axes[i, j].pcolormesh(xx1km, yy1km, ma.masked_less(radar_data, 10), cmap=CVD_cmap,
                                               norm=Normalize(-1, 80), linewidth=0, antialiased=True)  # , alpha=0.9)
            colors = ['#FF9494', '#FF5F5F', '#DC1C1C', '#920000']
            values = [0.1, 0.3, 0.5, 0.8]
            linewidths = [0.2, 0.4, 0.6, 0.8]
            labels = [str(int(v * 100)) + '%' for v in values]
            contour_img = axes[i, j].contour(xx1km, yy1km, preds, values, colors=colors,
                                             linewidths=linewidths)  # , cmap=plt.get_cmap('cividis'))
            custom_lines = [Line2D([0], [0], color=c, lw=lw * 2) for c, lw in zip(colors, linewidths)]
        elif c == 'C13':
            ...
            print('Max C13', np.nanmax(C13vals))
            print('Min C13', np.nanmin(C13vals))
            colors = ['#FF9494', '#FF5F5F', '#DC1C1C', '#920000']
            values = [0.2, 0.4, 0.6, 0.8]
            linewidths = [2, 2.2, 2.4, 2.8]
            labels = [str(int(v * 100)) + '%' for v in values]
            contour_img = axes[i, j].contour(xx1km, yy1km, preds, values, colors=colors,
                                             linewidths=linewidths)  # , cmap=plt.get_cmap('cividis')) #[::4,::4]
            custom_lines = [Line2D([0], [0], color=c, lw=lw * 2) for c, lw in zip(colors, linewidths)]
            # point = axes[i, j].plot(x_point, y_point, marker='*', color='darkblue', markersize=12) #Madison point
        else:
            img = axes[i, j].pcolormesh(xxhalfkm, yyhalfkm, C02vals, vmin=0, vmax=100, cmap=matplotlib.colormaps['Greys_r'])
            radimg_10C = axes[i, j].pcolormesh(xxhalfkm, yyhalfkm, ma.masked_less(zoom(radar_data, 2, order=0), 10),
                                               cmap=CVD_cmap, norm=Normalize(-1, 80), linewidth=0,
                                               antialiased=True)  # , alpha=0.9)
            colors = ['#FF9494', '#FF5F5F', '#DC1C1C', '#920000']
            values = [0.2, 0.4, 0.6, 0.8]
            linewidths = [1.6, 1.6, 1.7, 1.8, 2]
            labels = [str(int(v * 100)) + '%' for v in values]
            contour_img = axes[i, j].contour(xxhalfkm, yyhalfkm, zoom(preds, 2, order=0), values, colors=colors,
                                             linewidths=linewidths)  # , cmap=plt.get_cmap('cividis'))
            custom_lines = [Line2D([0], [0], color=c, lw=lw * 2) for c, lw in zip(colors, linewidths)]
        axes[i, j].set_title((' ').join([timestamp, 'UTC']), fontsize=16)
    return img, radimg_10C, custom_lines, labels
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', '--model', help='Model checkpoint to use.', type=str, nargs=1,
                        default=['/ships19/grain/convective_init/models/TC_CHs_02_05_13_15/thundercast-ckpt-epoch=29-val_loss_epoch=0.4488.ckpt']) #Night_Trial1/thundercast-ckpt-epoch=15-val_loss_epoch=0.4614.ckpt'
    ...
    if numplots % 2 != 0:
        ...
        print('Number of subplots should be adjusted.')
    elif straight:
        numcols = numplots
        numrows = 1
    else:
    ...
    for i in range(numrows):
        for j in range(numcols):
            ...
            preds[:, :] = raw_preds[0, 1, :, :]
            img, radimg_10C, custom_lines, labels = make_subplot(scn, preds, center_x_idx, center_y_idx, hs1km, radar_file, c, timestamp, i, j, fig, axes, geoproj, straight)
    ...
    if c == 'C05':
        ...
        cbar = plt.colorbar(img, orientation='horizontal', pad=0.01, extendfrac=0, drawedges=0,
                            ticks=np.arange(200, 305, 20), cax=cbaxes)
        cbar.set_label('ABI 10.3-$\mu$m BT [K]', fontsize=14)
    else:
        cbar = plt.colorbar(img, orientation='horizontal', pad=0.01, extendfrac=0, drawedges=0,
                            ticks=np.arange(0, 100, 10), cax=cbaxes)
        cbar.set_label('ABI 0.64-$\mu$m reflectance [%]', fontsize=14)
    ...
    if numplots == 9:
        leg2 = fig.legend(custom_lines, labels, fontsize=12, loc='lower right', handlelength=1.5,
                          ncol=len(labels), title='0-60 min. Thunderstorm Probability', title_fontsize=14,
                          bbox_to_anchor=(0.675, .048, 0.23, 0.04))
    else:
        leg2 = fig.legend(custom_lines, labels, fontsize=12, loc='lower right', handlelength=1.5,
                          ncol=len(labels), title='0-60 min. Thunderstorm Probability', title_fontsize=14,
                          bbox_to_anchor=(0.675, .024, 0.23, 0.04))
