...
from PL_Metrics import POD, FAR, SR, Accuracy, Specificity, F1Score, Recall, Precision, CEFs, NoRes
...
def get_abifile_lists(rootdatadir, dts): #get lists of abi files for datetimes
    list = []
...
def format_inputs(scn, norm_vals, vars, center_x_idx, center_y_idx, hs1km, flip): #take satellite data and format it for use in the model
    X = []
    for res_group in vars:
        if res_group == 'target':
            ...
            for variable in vars[res_group]:
                ...
                if flip == 'flip_0':
                    tmp = torch.flip(tmp, [0])
                elif flip == 'flip_1':
                    tmp = torch.flip(tmp, [1])
                elif flip == 'flip_01':
                    tmp = torch.flip(tmp, [0, 1])
                tmp = tmp[None]
                if x is None:
                    x = tmp + 0.
                else:
...
class UNet2D_LitModel(pl.LightningModule): #define the model
    def __init__(self, params):
        ...
        #Need the following by threshold: (delete any that aren't needed for testing, i.e. if you don't want an attributes or performance diagram)
        #CEFs
...
def get_radar_list(dts, rootradardir): #get list of radar files for datetimes
    radar_file_list = []
    for dt in dts:
...
#make a figure with a single timestamp of satellite data, radar, and ThunderCast predictions.
def make_figure(scn, preds, center_x_idx, center_y_idx, hs, radar_file, c, timestamp, figure_path, flip):
    ...
    C05vals = torch.tensor(cropped_scn['C05'].values)
    C13vals = torch.tensor(cropped_scn['C13'].values)
    C02vals = torch.tensor(cropped_scn['C02'].values)

    if flip == 'flip_0':
        C05vals = torch.flip(C05vals, [0])
        C13vals = torch.flip(C13vals, [0])
        C02vals = torch.flip(C02vals, [0])
    elif flip == 'flip_1':
        C05vals = torch.flip(C05vals, [1])
        C13vals = torch.flip(C13vals, [1])
        C02vals = torch.flip(C02vals, [1])
    elif flip == 'flip_01':
        C05vals = torch.flip(C05vals, [0, 1])
        C13vals = torch.flip(C13vals, [0, 1])
        C02vals = torch.flip(C02vals, [0, 1])
    ...
    if flip == 'flip_0':
        radar_data = torch.flip(radar_data, [0])
    elif flip == 'flip_1':
        radar_data = torch.flip(radar_data, [1])
    elif flip == 'flip_01':
        radar_data = torch.flip(radar_data, [0, 1])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-DT', '--DT', help='A datetime in %Y-%m-%d-%H-%M',
                        type=str, nargs=1)
    parser.add_argument('-m', '--model', help='Model checkpoint to use.', type=str, nargs=1, default=['/ships19/grain/convective_init/models/TC_CHs_02_05_13_15/thundercast-ckpt-epoch=29-val_loss_epoch=0.4488.ckpt'])
    ...
    parser.add_argument('-flip0', '--flip0', help='If you want to flip the image/preds on axis 0', action="store_true")
    parser.add_argument('-flip1', '--flip1', help='If you want to flip the image/preds on axis 1', action="store_true")
    parser.add_argument('-flip01', '--flip01', help='If you want to flip the image/preds on axis 1', action="store_true")
    ...
    for k in range(len(abifile_lists)):
        ...
        inputs = format_inputs(scn, norm_vals, vars, center_x_idx, center_y_idx, hs1km, flip)
        inputs = [inputs]
        ...
        print('Time to calculate preds:', time()-t_pred_start)
        print('pred shape before', raw_preds.shape)
        preds[:, :] = raw_preds[0, 1, :, :]
        print('pred shape:', preds.shape)
        print('pred max and min:', np.nanmax(preds), np.nanmin(preds))
        ...
        if flip == None:
            figure_path = rootpreddir + '/preds_' + time_nm + '_' + center_lat_nm + '_' + center_lon_nm + '_' + str(hs1km) + '.png'
        else:
            figure_path = rootpreddir + '/preds_' + time_nm + '_' + center_lat_nm + '_' + center_lon_nm + '_' + str(hs1km) + '_' + flip + '.png'
        make_figure(scn, preds, center_x_idx, center_y_idx, hs1km, radar_file, c, timestamp, figure_path, flip)
